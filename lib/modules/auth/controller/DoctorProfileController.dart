...
class DoctorProfileController extends GetxController {
  ...
  Future<void> updateAvailabilityStatus(String status) async {
    try {
      ...
      if (response.status == "success" && doctor.value != null) {
        doctor.value =
            doctor.value!.copyWith(availabilityStatus: status);
      }
    } finally {
      isUpdatingAvailability.value = false;
    }
  ...
  try {
    ...
    final response = await _repo.uploadProfileImageInBase64(
    ...
    if (response.status == "success") {
      final imageUrl = response.uploadInfo?.location;
      if (imageUrl != null && doctor.value != null) {
        doctor.value = doctor.value!.copyWith(photo: imageUrl);
      }
    }
  } finally {
    isUploadingImage.value = false;
  ...
  //       final imageUrl = response.uploadInfo?.location;
  //       if (imageUrl != null && doctor.value != null) {
  //         /// ✅ FIX: OBJECT REASSIGN (NO refresh())
  //         doctor.value = doctor.value!.copyWith(photo: imageUrl);
  //       }
  //     }
  //   } finally {
...
Future<void> _pick(ImageSource source) async {
  ...
  await uploadProfileImage(base64Image, extension); // ✅ FIX
}
